...
window.addEvent('domready',function (){
    ...
    list.onBeforeDeleteRow = function(row){
        ...
        var ret = true;
        new Request({
            url: '/senna/clientes/clientes/antesExcluir/'+row.id,
            async: false,
            onComplete: function(response) {
                if (response == '1') {
                    Sexy.alert("<h5>Atenção:</h5><br/><div style='max-height:270px;overflow:auto;display:block;'>Cliente não pode ser excluido pois já possui vinculos dentro do sistema.Você pode acessar o cliente e desativa-lo.</div>");
                    ret = false;
                }
            }
        }).send();
